...
class ChatChannelUnreadState(
    ...
    fun markUnreadForMessage(
        ...
        channels.forEach { channel ->
            ...
            if (!ChatChannelRules.matches(text, channel, channels)) {
                return@forEach
            }
            if (chatOpen && channel.id == activeChannelId) {
                return@forEach
            }
    ...
    fun hasUnreadChannel(channelId: String): Boolean {
        return channelId in unreadChannelIds
    }

    fun hasAnyUnread(): Boolean {
        return unreadChannelIds.isNotEmpty()
    }
}
